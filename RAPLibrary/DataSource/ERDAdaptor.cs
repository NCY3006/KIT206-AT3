...
namespace RAPLibrary.DataSource
{
    abstract class ERDAdapter
    {
        ...
        private static void GetConnection()
        {
            ...
            if (db_conn == null)
            {
                // string connectionString = "Server=localhost;Database=kit206;Uid=root;Pwd='';";
        ...
        public static List<Entities.Publication> fetchBasicPublicationDetails(Entities.Researcher researcher)
        {
            ...
            foreach (string doi in dois)
            {
                ...
                try
                {
                    ...
                    // Create command, including database connection
                    MySqlCommand cmd = new MySqlCommand("SELECT title, year, ranking FROM publication WHERE doi = ?doi", db_conn);
                    ...
                    while (rdr.Read())
                    {
                        publications.Add(new Entities.Publication
                        {
                            ...
                            Ranking = (rdr.GetString(2) == "Q1") ? 1 : (rdr.GetString(2) == "Q2") ? 2 : (rdr.GetString(2) == "Q3") ? 3 : 4
                        });
                    }
        ...
        public static Entities.Publication fetchFullPublicationDetails(Entities.Publication publication)
        {
            ...
            try
            {
                ...
                publication.Ranking = (rdr.GetString(2) == "Q1") ? 1 : (rdr.GetString(2) == "Q2") ? 2 : (rdr.GetString(2) == "Q3") ? 3 : 4;
                publication.Type = ParseEnum<Entities.OutputType>(rdr.GetString(5));
                publication.Authors = rdr.GetString(3);
                publication.CiteAs = rdr.GetString(6);
